...
const SnakeLibrary: React.FC<{ language: Language }> = ({ language }) => {
  ...
  const filteredSnakes = FREEDOMLAND_SNAKES.filter(s => {
    ...
    const matchesSearch = s.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          ...
                          s.vernacularNames.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 relative">
      <div className="mb-8 text-center max-w-2xl mx-auto">
        ...
        <p className="text-gray-500 text-sm md:text-base mt-2">
          Discover the {FREEDOMLAND_SNAKES.length} native species.
        </p>
      </div>
        ...
        {filteredSnakes.map(snake => (
          ...
          >
            <div className="aspect-[4/3] overflow-hidden relative bg-gray-50">
              <img src={snake.imageUrl} alt={snake.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" />
            </div>
            <div className="p-3.5 flex flex-col flex-grow">
              <h3 className="text-sm font-bold text-gray-900 leading-tight truncate">{snake.name}</h3>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md" onClick={closeModal}>
          <div className="bg-white rounded-[32px] w-full max-w-2xl overflow-hidden shadow-2xl max-h-[95vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="relative h-64 md:h-80 flex-shrink-0">
              <img src={selectedSnake.imageUrl} alt={selectedSnake.name} className="w-full h-full object-cover" />
              <button onClick={closeModal} className="absolute top-4 right-4 bg-black/40 text-white w-10 h-10 rounded-full flex items-center justify-center"><i className="fa-solid fa-xmark"></i></button>
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-transparent to-transparent text-white">
                <h2 className="text-2xl md:text-4xl font-serif font-bold">{selectedSnake.name}</h2>
              </div>
            </div>
            <div className="p-6 md:p-10 overflow-y-auto space-y-8">
              <div className="flex flex-wrap gap-4 text-xs text-gray-400 font-bold uppercase tracking-widest">
                <span>{t.avg}: {selectedSnake.averageLength}</span>
                <span>{t.max}: {selectedSnake.maxLength}</span>
              </div>
              <div className="grid md:grid-cols-5 gap-8">
                <div className="md:col-span-3 space-y-6">
                  <section>
                    <h4 className="text-[10px] font-black text-green-800 uppercase tracking-widest mb-2.5">{t.overview}</h4>
                    <p className="text-gray-700 text-sm leading-relaxed">{selectedSnake.description}</p>
                  </section>
                  <section>
                    <h4 className="text-[10px] font-black text-green-800 uppercase tracking-widest mb-2.5">{t.keyID}</h4>
                    <p className="text-gray-700 text-sm leading-relaxed italic border-l-2 border-green-100 pl-4">{selectedSnake.keyFeatures}</p>
                  </section>
                </div>
                <div className="md:col-span-2 space-y-4">
                  <div className="bg-gray-50 p-4 rounded-xl">
                    <h5 className="text-[9px] font-black text-gray-400 uppercase mb-1">{t.habitat}</h5>
                    <p className="text-xs font-bold">{selectedSnake.habitat}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-xl">
                    <h5 className="text-[9px] font-black text-gray-400 uppercase mb-1">{t.distribution}</h5>
                    <p className="text-xs font-bold">{selectedSnake.distribution}</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="p-6 border-t border-gray-100">
              <button onClick={closeModal} className="w-full py-4 bg-green-900 text-white rounded-2xl font-bold text-sm uppercase">{t.close}</button>
            </div>
          </div>
        </div>
      )}
